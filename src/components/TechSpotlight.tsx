...
import { Cpu, QrCode, ClipboardList, Thermometer, Radio, Play } from "lucide-react";
...
export default function TechSpotlight() {
  ...
  return (
    <section id="tech" className="relative py-24 bg-[#070b19] overflow-hidden border-t border-b border-white/5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            ...
            <div className="space-y-3">
              <button
                ...
                className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all duration-300 text-left cursor-pointer ${
                  activeSystem === "patrol"
                    ? "bg-slate-900 border-sky-500/50 text-white shadow-lg shadow-sky-500/10"
                    : "border-white/5 bg-slate-900/40 text-slate-400 hover:text-white hover:border-white/15 hover:bg-slate-900/60"
                }`}
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold">QR & NFC Logs</p>
                  </div>
                </div>
                <Radio className={`w-3.5 h-3.5 ${activeSystem === "patrol" ? "text-sky-400 animate-pulse" : "text-slate-600"}`} />
              </button>
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold">Mobile Tickets</p>
                  </div>
                </div>
                <Radio className={`w-3.5 h-3.5 ${activeSystem === "mep" ? "text-sky-400 animate-pulse" : "text-slate-600"}`} />
              </button>
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold">STP Telemetry</p>
                  </div>
                </div>
                <Radio className={`w-3.5 h-3.5 ${activeSystem === "iot" ? "text-sky-400 animate-pulse" : "text-slate-600"}`} />
              </button>
            </div>

            {/* Quick Tech Specs list */}
            <div className="p-5 rounded-xl border border-white/5 bg-slate-950/60 space-y-3">
              ...
              <ul className="space-y-2">
                {systems[activeSystem].techs.map((tech, idx) => (
                  <li key={idx} className="flex items-center space-x-2 text-xs text-slate-300 font-semibold">
          ...
          <div className="lg:col-span-8 flex flex-col justify-between rounded-2xl glass-panel p-6 border border-white/10 bg-slate-900/30 min-h-[400px]">
                ...
                <button
                  ...
                  className={`flex items-center space-x-1.5 px-4 py-2 rounded-xl text-xs font-bold border transition-all duration-300 cursor-pointer ${
                    isSimulating
                      ...
                      : "bg-sky-500/10 border-sky-500/20 text-sky-400 hover:bg-sky-500/25"
                  }`}
                >
            ...
            <div className="bg-black/90 rounded-xl border border-white/10 p-4 font-mono text-[11px] text-sky-400 space-y-1.5 max-h-48 overflow-y-auto shadow-inner relative">
