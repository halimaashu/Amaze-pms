...
export default function Calculator() {
  ...
  return (
    <section id="estimator" className="relative py-24 bg-[#040711] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          {/* Inputs - Left Column */}
          <div className="lg:col-span-7 rounded-2xl glass-panel p-6 md:p-8 border border-white/10 bg-slate-900/30 space-y-6">
            <h4 className="text-lg font-bold text-white">1. Property Specifications</h4>

            {/* Property Type Radio Selector */}
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wide">Property Category</label>
                  ...
                  <button
                    ...
                    className={`px-3 py-2.5 rounded-lg text-xs font-bold border transition-colors cursor-pointer ${
                      propertyType === item.key
                        ? "bg-sky-500/20 border-sky-500/40 text-white shadow-md shadow-sky-500/10"
                        : "border-white/5 bg-slate-900/40 text-slate-400 hover:text-white hover:border-white/15"
                    }`}
                  >
                    {item.label}
                  </button>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-xs font-bold text-slate-400 uppercase tracking-wide">
                ...
                <span className="text-sky-400 text-sm font-mono">{area.toLocaleString()} Sq Ft</span>
              </div>
              <input
                type="range"
                min="10000"
                ...
                className="w-full h-2 bg-slate-900 rounded-lg appearance-none cursor-pointer accent-sky-400 border border-white/5"
              />
            ...
            {/* Service Checkboxes */}
            <div className="space-y-3">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block">Select Required Modules</label>
                  ...
                  <div
                    ...
                    className={`p-4 rounded-xl border cursor-pointer select-none transition-all duration-300 ${
                      (services as any)[item.key]
                        ? "bg-[#0c1225] border-sky-500/40 shadow-md"
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <input
                        ...
                        readOnly
                        className="w-4 h-4 rounded border-slate-700 bg-slate-900 text-sky-400 focus:ring-0 focus:ring-offset-0 pointer-events-none"
                      />
                      <div>
          ...
          <div className="lg:col-span-5 flex flex-col justify-between rounded-2xl glass-panel p-6 md:p-8 border border-white/10 bg-slate-900/25">
            <div className="space-y-6">
              <h4 className="text-lg font-bold text-white border-b border-white/5 pb-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950/80 p-4 rounded-xl border border-white/5">
                  ...
                  <p className="text-3xl font-extrabold text-white mt-1.5">{estimate.staffCount}</p>
                ...
                <div className="bg-slate-950/80 p-4 rounded-xl border border-white/5">
                  ...
                  <p className="text-3xl font-extrabold text-white mt-1.5">{estimate.supervisors}</p>
                  <p className="text-[9px] text-slate-500 mt-1">Quality audits managers</p>
                </div>
              </div>

              <div className="space-y-3.5 pt-2">
                <div className="flex justify-between items-center text-xs p-3 rounded-lg bg-slate-950/40 border border-white/5">
                  <span className="text-slate-400 font-medium">Response SLA Guarantee</span>
                  <span className="font-bold text-sky-400">&lt; {estimate.slaMins} Mins</span>
                </div>

                <div className="flex justify-between items-center text-xs p-3 rounded-lg bg-slate-950/40 border border-white/5">
                  <span className="text-slate-400 font-medium">Statutory Compliance</span>
                  <span className="font-bold text-emerald-400">{estimate.complianceScore}</span>
                </div>

                <div className="flex justify-between items-center text-xs p-3 rounded-lg bg-slate-950/40 border border-white/5">
                  <span className="text-slate-400 font-medium">Management Auditing</span>
                  <span className="font-bold text-white">Bi-Weekly ISO Standard</span>
                </div>
              </div>
            </div>
